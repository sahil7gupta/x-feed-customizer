...
if (typeof chrome !== 'undefined' && chrome.runtime) {
  ...
  const XFeedCustomizer = {
    ...
    init: async function() {
      ...
      try {
        ...
        if (result.feedSettings) {
          ...
          console.log('Classification method:', this.settings.classificationMethod || 'keywords (default)');
    ...
    processPost: async function(postElement) {
      ...
      // Classify post text using the selected method (keywords or API)
      let category;
      try {
        // Check which classification method to use
        if (!this.settings.classificationMethod || this.settings.classificationMethod === 'keywords') {
          // Use keyword classification
          category = this.keywordClassify(text);
          console.log('POST CATEGORY (Keywords):', category);
        } else if (this.settings.classificationMethod === 'api') {
          ...
          // Fallback to keywords if method is unknown
          category = this.keywordClassify(text);
          console.log('Unknown classification method, using keywords. Category:', category);
        }
    ...
    // A simple classifier based on keywords
    keywordClassify: function(text) {
      if (!text) return 'Entertainment';
      
      text = text.toLowerCase();
      
      // Check engagement bait patterns
      if (
        text.includes('retweet if') || 
        text.includes('like if') || 
        text.includes('share if') ||
        text.includes('agree?') ||
        text.includes('rt to') ||
        text.includes('raise your hand if') ||
        text.includes('rt for') ||
        text.includes('do you agree') ||
        text.includes('do you think') ||
        text.includes('thoughts?') ||
        text.includes('who else thinks') ||
        (text.split(' ').length < 5 && !text.includes('http')) // Very short posts without links
      ) {
      ...
      const categoryKeywords = {
        'Technology': ['tech', 'software', 'hardware', 'app', 'digital', 'computer', 'smartphone', 'device', 'gadget', 'programming', 'code', 'developer', 'engineering', 'browser', 'internet', 'web development', 'algorithm', 'database', 'cyber', 'tech stack', 'git', 'programming language'],
        
        'AI/ML': ['ai', 'artificial intelligence', 'machine learning', 'neural', 'gpt', 'llm', 'deep learning', 'data science', 'chatgpt', 'claude', 'openai', 'tensor', 'transformer', 'model', 'training data', 'generative', 'ml', 'nlp', 'computer vision', 'dall-e', 'midjourney', 'stable diffusion', 'anthropic', 'language model', 'prediction model'],
        
        'Science': ['science', 'research', 'study', 'discovery', 'experiment', 'scientist', 'physics', 'chemistry', 'biology', 'astronomy', 'medicine', 'mathematical', 'theory', 'hypothesis', 'laboratory', 'molecular', 'quantum', 'academic', 'journal', 'peer review', 'clinical', 'genetic', 'ecosystem'],
        
        'Business': ['business', 'company', 'ceo', 'market', 'profit', 'revenue', 'investment', 'finance', 'economy', 'stock', 'shareholders', 'corporate', 'industry', 'management', 'enterprise', 'commercial', 'trade', 'acquisition', 'merger', 'consumer', 'retail', 'manufacturing', 'economic'],
        
        'Startups': ['startup', 'founder', 'funding', 'series a', 'venture', 'vc', 'seed round', 'launch', 'entrepreneur', 'angel investor', 'incubator', 'accelerator', 'pitch', 'valuation', 'disrupt', 'innovation', 'mvp', 'product market fit', 'growth hacking', 'bootstrapped', 'unicorn', 'exit strategy'],
        
        'Crypto': ['crypto', 'bitcoin', 'ethereum', 'blockchain', 'nft', 'token', 'defi', 'web3', 'wallet', 'mining', 'altcoin', 'satoshi', 'btc', 'eth', 'ledger', 'decentralized', 'hashrate', 'exchange', 'smart contract', 'dao', 'staking', 'binance', 'coinbase'],
        
        'Politics': ['politics', 'government', 'election', 'president', 'vote', 'congress', 'democrat', 'republican', 'policy', 'senate', 'parliament', 'campaign', 'political', 'candidate', 'voter', 'legislation', 'governor', 'constitutional', 'democracy', 'conservative', 'liberal', 'progressive', 'administration', 'trump', 'biden', 'harris'],
        
        'World News': ['news', 'world', 'crisis', 'breaking', 'report', 'announce', 'global', 'international', 'headline', 'press', 'journalist', 'correspondent', 'broadcast', 'coverage', 'bulletin', 'reporter', 'media', 'newspaper', 'current events', 'incident', 'development'],
        
        'Health': ['health', 'medical', 'doctor', 'patient', 'hospital', 'disease', 'treatment', 'wellness', 'fitness', 'exercise', 'diet', 'nutrition', 'mental health', 'therapy', 'healthcare', 'diagnosis', 'symptom', 'medication', 'workout', 'wellbeing', 'obesity', 'calories', 'syndrome'],
        
        'Education': ['education', 'school', 'student', 'learn', 'teach', 'professor', 'class', 'course', 'knowledge', 'lecture', 'academic', 'university', 'college', 'curriculum', 'lesson', 'instructor', 'degree', 'dissertation', 'thesis', 'classroom', 'pedagogy', 'discipline', 'scholarship'],
        
        'Art/Design': ['art', 'design', 'creative', 'artist', 'painting', 'drawing', 'museum', 'gallery', 'aesthetic', 'illustration', 'visual', 'exhibition', 'sculpture', 'photography', 'graphic design', 'typography', 'craft', 'artistic', 'portrait', 'composition', 'sketch', 'collage', 'curator'],
        
        'Gaming': ['game', 'gaming', 'play', 'player', 'console', 'playstation', 'xbox', 'nintendo', 'steam', 'gamer', 'esports', 'multiplayer', 'rpg', 'fps', 'mmorpg', 'twitch', 'streaming', 'controller', 'gameplay', 'character', 'level', 'quest', 'leaderboard', 'achievement'],
        
        'Food': ['food', 'recipe', 'cook', 'restaurant', 'meal', 'dish', 'chef', 'delicious', 'eat', 'taste', 'cuisine', 'flavor', 'baking', 'ingredient', 'kitchen', 'culinary', 'dining', 'pizza', 'burger', 'dessert', 'appetizer', 'menu', 'grill', 'vegetarian', 'vegan'],
        
        'Travel': ['travel', 'trip', 'vacation', 'destination', 'journey', 'hotel', 'flight', 'tourist', 'visit', 'tourism', 'passenger', 'resort', 'beach', 'adventure', 'sightseeing', 'itinerary', 'backpacking', 'cruise', 'excursion', 'traveler', 'landmark', 'guide', 'exploration']
      };
      ...
      // If no significant matches, default to Entertainment
      return maxScore > 0 ? bestCategory : 'Entertainment';
    },
